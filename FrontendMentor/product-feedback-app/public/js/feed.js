class DadosMessage {
    constructor (id, upvotes, title, description, category, tatalcomments) {
        this.id = id
            ...
            dados.productRequests.map((message) => {
                if (category == 'All') {
                    if ('comments' in message) {
                        ...
                        let objMessage = new DadosMessage(message.id, message.upvotes, message.title, message.description, message.category, totalComments)
                        orgMsg.push(objMessage)
                    } else {
                        let objMessage = new DadosMessage(message.id, message.upvotes, message.title, message.description, message.category, 0)
                        orgMsg.push(objMessage)
                    }
                }
                ...
                for (let msg of orgMsg) {
                    if (msg.tatalcomments === 0) {
                        ...
                        <div class="comment">
                            ...
                            <a href="feedback-detail.html?id=${msg.id}" class="comments">
